...
export default function FormWebs({ exitingData, onClose }: WebsFormProps) {
  ...
  const [category, setCategory] = useState(
    exitingData?.category || "Web Development"
  );
  ...
  return (
    <form
      ...
      className="bg-white p-4 rounded-lg shadow-md mb-6 border-b-8"
    >
      ...
      <input
        ...
        value={titleWebs}
        onChange={(e) => setTitleWebs(e.target.value)}
        required
        disabled={!!exitingData}
        className="w-full p-2 border mb-2"
      />
      <h1>Link Project Web</h1>
      <input
        type="text"
        placeholder="Masukan Link Project"
        value={linkFull}
        onChange={(e) => setLinkFull(e.target.value)}
        required
        disabled={!!exitingData}
        className="w-full p-2 border mb-2"
      />
